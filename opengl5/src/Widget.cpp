#include"Widget.h"
Widget::Widget(std::shared_ptr<MyShader>shader, std::shared_ptr<BufferData>buffer):
	m_Shader(shader),
	m_Buffer(buffer)
{
	InitWidget();
...
void Widget::Show()
{
	...
	while (!glfwWindowShouldClose(window))
	{
        ...
        float scaleAmount = static_cast<float>(abs(sin(glfwGetTime() * 0.5)));
...
void Widget::DrawArrary()
{
	if (m_Shader != nullptr && m_Buffer != nullptr)
	{
		glBindTexture(GL_TEXTURE_2D, m_Buffer->Gettexture());
		m_Shader->use();
